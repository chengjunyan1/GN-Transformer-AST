...
def prepare_batch(batch, model):
    ...
    da = batch['da']
    nids = batch['nids']
    rpe_mask = batch['rpe_mask']
    wtype=None
    if args.cuda:
        code_len = batch['code_len'].cuda(non_blocking=True)
        wpos = batch['wpos'].cuda(non_blocking=True)
        astok = batch['astok'].cuda(non_blocking=True)
        if model.args.use_word_type:
            wtype = batch['wtype'].cuda(non_blocking=True)
    ...
    batch_inputs['seqlen'] = seqlen
    batch_inputs['wpos'] = wpos
    batch_inputs['astok'] = astok
    batch_inputs['bg'] = bg
    batch_inputs['dm'] = dm
    batch_inputs['da'] = da
    batch_inputs['nids'] = nids
    batch_inputs['rpe_mask'] = rpe_mask
    batch_inputs['wtype'] = wtype
    return batch_inputs
...
def main(args):
    ...
    for dev_src, dev_tgt, dev_ast, dataset_name in \
            zip(args.dev_src_files, args.dev_tgt_files,
                args.dev_ast_files, args.dataset_name):
        dev_files = dict()
